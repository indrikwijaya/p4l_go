...
//It returns a gravity simulation over the appropriate number of generations, estimated for the given time interval (in seconds).
...
func UpdateUniverse(currentUniverse Universe, time float64) Universe {
    ...
    // newUniverse.bodies has type []Body and contains bodies we want to access
...
//UpdateAcceleration takes a slice of Body objects and a Body object and compute the acceleration of the body object based on the force of gravity of evertyhing in the universe.
...
//ComputeNetForce takes a slice of Body object and a separate body object. It returns the net force of gravity (as an ordered pair) that the bodies exert on the given Body object.
...
//ComputeForce takes two bodies and computes the force exerted due to gravity on the second object due to the first.
func ComputeForce(b2, b Body) OrderedPair {
    ...
    //apply formula for magnitude of force
...
func CopyUniverse(currentUniverse Universe) Universe {
    ...
    // this wont duplicate the bodies ... it would give me the same bodies: newUniverse.bodies = currentUniverse.bodies
    ...
    for i := range currentUniverse.bodies {
        var b Body
        // set the fields of b equal to those of current body (currentUniverse.bodies[i])
