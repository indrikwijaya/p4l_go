...
func UpdateCell(board GameBoard, r, c int, neighborhood string, ruleStrings []string) int {
    ...
    for _, rule := range ruleStrings {
        if RuleMatch(board, r, c, neighborhood, rule) == true {
            // match found! return last element of current tule string
            finalSymbol := string(rule[len(rule)-1])
            // convert finalSymbol to int
            newState, err := strconv.Atoi(finalSymbol)
            if err != nil {
                panic("Error: problem in converting final rule in string symbol.")
            }
            return newState
        }
    }
...
//RuleMatch takes a GameBoard, row/col indices r, c, a neighborhood parameter(vonNeumann or Moore), and a rule string.
...
func RuleMatchMoore(board GameBoard, r, c int, rule string) bool {
    ...
    north := strconv.Itoa(board[r-1][c])
    northEast := strconv.Itoa(board[r-1][c+11])
    east := strconv.Itoa(board[r][c+1])
    ...
    // see if any neighborhood cell matches the rule
    if center != string(rule[0]) ||
    northWest != string(rule[1]) ||
    north != string(rule[2]) ||
    northEast != string(rule[3]) ||
    east != string(rule[4]) ||
    southEast != string(rule[5]) ||
    south != string(rule[6]) ||
    southWest != string(rule[7]) ||
    west != string(rule[8]) {
        return false
    }
    // if we survived, return true
    return true
}
